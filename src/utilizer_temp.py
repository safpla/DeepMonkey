    ...
    for filename in table_filenames:
        ...
        for line in fin.readlines():
            ...
            if len(line_split) > 8:
                ...
                call_type = 'Other'
            call = call_class.Call(subject, session_num, int(line_split[2]), float(line_split[3]), float(line_split[4]),call_type)
...
def write_to_selectionTable(table_filenames, call_segs):
    for table_filename, call_seg in zip(table_filenames, call_segs):
        ...
        for ind, call in enumerate(call_seg):
            line = '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s' %(ind, 'Spectrogram 1', call.channel_num, call.begin_time, call.end_time, 4000.0, 18000.0, 18000.0, call.call_type)
            fout.write(headline)
